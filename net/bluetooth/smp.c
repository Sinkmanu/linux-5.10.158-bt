...
//int smp_max_enc_key_size __read_mostly;
//int smp_max_enc_key_size __read_mostly = 16;
...
static void smp_send_cmd(struct l2cap_conn *conn, u8 code, u16 len, void *data)
{
	...
	printk(KERN_INFO "NIAP-CC: smp_send_cmd; code 0x%2.2x\n", code);

	// NIAP-CC: Modified
	/*if (!chan){
		return;
	} else {
		printk(KERN_INFO "NIAP-CC: smp_send_cmd; no chan\n");
	}*/
			      ...
			      struct smp_cmd_pairing *rsp, __u8 authreq)
{
	...
	// NIAP-CC
	printk(KERN_INFO "NIAP-CC: build_pairing_cmd\n");
	...
	if (rsp == NULL) {
		...
		printk(KERN_INFO "NIAP-CC: Overwriting rsp->max_key_size = %d\n", rsp->max_key_size);
...
static u8 check_enc_key_size(struct l2cap_conn *conn, __u8 max_key_size)
{
 ...
 // NIAP-CC patch
	...
	// NIAP-CC Patch
	printk(KERN_INFO "NIAP-CC: smp->enc_key_size old: %d\n", max_key_size);
	smp->enc_key_size = max_key_size;
	printk(KERN_INFO "NIAP-CC: smp->enc_key_size new: %d\n", smp_max_enc_key_size);
	smp->enc_key_size = smp_max_enc_key_size;
						...
						u8 local_io, u8 remote_io)
{
	...
	if (smp->method == CFM_PASSKEY) {
		...
		printk(KERN_INFO "NIAP-CC: PassKey: %d\n", passkey);
...
static u8 smp_confirm(struct smp_chan *smp)
{
	...
	BT_DBG("conn %p", conn);
	printk(KERN_INFO "NIAP-CC: smp_confirm\n");
...
static u8 smp_random(struct smp_chan *smp)
{
	...
	printk(KERN_INFO "NIAP-CC: smp_random\n");
	...
	if (hcon->out) {
	 ...
	 printk(KERN_INFO "NIAP-CC: hci_le_start_enc: smp->enc_key_size: %d\n", smp->enc_key_size);
...
static void smp_distribute_keys(struct smp_chan *smp)
{
    ...
    printk(KERN_INFO "NIAP-CC: keydist 0x%x\n", *keydist);
...
static struct smp_chan *smp_chan_create(struct l2cap_conn *conn)
{
	...
	printk(KERN_INFO "NIAP-CC: smp_chan_create\n");
...
static void sc_dhkey_check(struct smp_chan *smp)
{
	...
	printk(KERN_INFO "NIAP-CC: sc_dhkey_check\n");
...
static u8 sc_passkey_round(struct smp_chan *smp, u8 smp_op)
{
	...
	case SMP_CMD_PAIRING_RANDOM:
		...
		printk(KERN_INFO "NIAP-CC: PassKey rounds are complete ' Start DHKey Check'\n");
		sc_dhkey_check(smp);
		SMP_ALLOW_CMD(smp, SMP_CMD_DHKEY_CHECK);
...
{
	printk(KERN_INFO "NIAP-CC: smp_user_confirm_reply\n");
				    ...
				    struct smp_cmd_pairing *rsp)
{
	...
	printk(KERN_INFO "NIAP-CC: build_bredr_pairing_cmd\n");
	...
	if (!rsp) {
		...
		printk(KERN_INFO "NIAP-CC: req->max_key_size: %d\n", req->max_key_size);
		req->max_key_size    = smp_max_enc_key_size;
    printk(KERN_INFO "NIAP-CC: New req->max_key_size: %d\n", req->max_key_size);
		smp->remote_key_dist = remote_dist;
	...
	printk(KERN_INFO "NIAP-CC: rsp->max_key_size req: %d\n", req->max_key_size);
	//rsp->max_key_size    = conn->hcon->enc_key_size;
	rsp->max_key_size    = smp_max_enc_key_size;
	printk(KERN_INFO "NIAP-CC: New rsp->max_key_size: %d\n", rsp->max_key_size);
	rsp->init_key_dist   = req->init_key_dist & remote_dist;
	rsp->resp_key_dist   = req->resp_key_dist & local_dist;
  smp->remote_key_dist = rsp->init_key_dist;
}
...
static u8 smp_cmd_pairing_req(struct l2cap_conn *conn, struct sk_buff *skb)
{
	...
	BT_DBG("conn %p", conn);
	printk(KERN_INFO "NIAP-CC: smp_cmd_pairing_req\n");
	...
	if (conn->hcon->type == ACL_LINK) {
		...
		printk(KERN_INFO "NIAP-CC: req max_key_size: %d\n", req->max_key_size);
		printk(KERN_INFO "NIAP-CC: rsp max_key_size: %d\n", rsp.max_key_size);
		rsp.max_key_size = smp_max_enc_key_size;
		printk(KERN_INFO "NIAP-CC: Overwriting rsp max_key_size: %d\n", rsp.max_key_size);
		printk(KERN_INFO "NIAP-CC: key_size: %d\n", key_size);
		printk(KERN_INFO "NIAP-CC: key_size from config: %d\n", smp_max_enc_key_size);
...
static u8 sc_send_public_key(struct smp_chan *smp)
{
	...
	printk(KERN_INFO "NIAP-CC: sc_send_public_key\n");
	...
	if (test_bit(SMP_FLAG_LOCAL_OOB, &smp->flags)) {
		...
		printk(KERN_INFO "NIAP-CC: Local Public Key X: %32phN\n", smp->local_pk);
	...
	if (hci_dev_test_flag(hdev, HCI_USE_DEBUG_KEYS)) {
		...
		while (true) {
			/* Generate key pair for Secure Connections */
			printk(KERN_INFO "NIAP-CC: Generating key pair for Secure Connections\n");
	...
	printk(KERN_INFO "NIAP-CC: Local Public Key X: %32phN\n", smp->local_pk);
	printk(KERN_INFO "NIAP-CC: Local Public Key Y: %32phN\n", smp->local_pk + 32);
...
static u8 smp_cmd_pairing_rsp(struct l2cap_conn *conn, struct sk_buff *skb)
{
	...
	BT_DBG("conn %p", conn);
	printk(KERN_INFO "NIAP-CC: smp_cmd_pairing_rsp\n");
...
static int smp_cmd_public_key(struct l2cap_conn *conn, struct sk_buff *skb)
{
	...
	BT_DBG("conn %p", conn);
	printk(KERN_INFO "NIAP-CC: smp_cmd_public_key\n");
	...
	printk(KERN_INFO "NIAP-CC: Remote Public Key X: %32phN\n", smp->remote_pk);
	printk(KERN_INFO "NIAP-CC: Remote Public Key Y: %32phN\n", smp->remote_pk + 32);
  ...
  printk(KERN_INFO "NIAP-CC: DHKey: %32phN\n", smp->dhkey);
  ...
  printk(KERN_INFO "NIAP-CC: name: %s\tmethod: 0x%02x\n", hdev->name, smp->method);
...
static void bredr_pairing(struct l2cap_chan *chan)
{
	...
	BT_DBG("chan %p", chan);
	printk(KERN_INFO "NIAP-CC: bredr_pairing\n");

	/* NIAP-CC Disable everything for testing */
	...
	if (!smp) {
		...
		printk(KERN_INFO "NIAP-CC: unable to create SMP context for BR/EDR\n");
		return;
	}
	...
	printk(KERN_INFO "NIAP-CC: %s starting SMP over BR/EDR\n", hdev->name);
	...
	printk(KERN_INFO "NIAP-CC: Sending BR/EDR pairing\n");
