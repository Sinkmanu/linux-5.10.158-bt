			      ...
			      void *data, u16 data_len)
{
	...
	bt_dev_dbg(hdev, "sock %p", sk);
	printk(KERN_INFO "NIAP-CC: read_security_info");
	...
	if (hdev->commands[41] & 0x08)
		...
		sec_len = eir_append_le16(rp->sec, sec_len, 0x02, smp_max_enc_key_size);

 /* NIAP-CC: modified with config */
 	printk(KERN_INFO "NIAP-CC: hdev->max_enc_key_size: %d\n", hdev->max_enc_key_size);
	printk(KERN_INFO "NIAP-CC: SMP_MAX_ENC_KEY_SIZE from config file: %d\n", smp_max_enc_key_size);
