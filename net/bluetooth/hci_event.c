					   ...
					   struct sk_buff *skb)
{
	...
	BT_DBG("%s status 0x%2.2x", hdev->name, rp->status);
	printk(KERN_INFO "NIAP-CC: hci_cc_read_local_pairing_opts\n");
	printk(KERN_INFO "NIAP-CC: rp->max_key_size: %d\n", rp->max_key_size);
	printk(KERN_INFO "NIAP-CC: hdev->max_enc_key_size: %d\n", hdev->max_enc_key_size);
	...
	if (hci_dev_test_flag(hdev, HCI_SETUP) ||
		...
		hdev->max_enc_key_size = smp_max_enc_key_size;
		printk(KERN_INFO "NIAP-CC: OVerwriting hdev->max_enc_key_size: %d\n", hdev->max_enc_key_size);
				       ...
				       u16 opcode, struct sk_buff *skb)
{
	...
	printk(KERN_INFO "NIAP-CC: read_enc_key_size_complete\n");
	...
	if (rp->status) {
		...
		printk(KERN_INFO "NIAP-CC: read_enc_key_size_complete, key_size: %d\n", conn->enc_key_size);
	}
...
static void hci_encrypt_change_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	BT_DBG("%s status 0x%2.2x", hdev->name, ev->status);
	printk(KERN_INFO "NIAP-CC: hci_encrypt_change_evt\n");
	...
	if (!ev->status) {
		if (ev->encrypt) {
			printk(KERN_INFO "NIAP-CC: hci_encrypt_change_evt - encrypt\n");
			...
			/* NIAP-CC
			*/
			switch (conn->sec_level){
				case BT_SECURITY_SDP:
					printk(KERN_INFO "NIAP-CC: hci_encrypt_change_evt - BT_SECURITY_SDP\n");
					break;

				case BT_SECURITY_LOW:
					printk(KERN_INFO "NIAP-CC: hci_encrypt_change_evt - BT_SECURITY_LOW\n");
					break;

				case BT_SECURITY_MEDIUM:
					printk(KERN_INFO "NIAP-CC: hci_encrypt_change_evt - BT_SECURITY_MEDIUM\n");
					break;

				case BT_SECURITY_HIGH:
					printk(KERN_INFO "NIAP-CC: hci_encrypt_change_evt - BT_SECURITY_HIGH\n");
					break;

				case BT_SECURITY_FIPS:
					printk(KERN_INFO "NIAP-CC: hci_encrypt_change_evt - BT_SECURITY_FIPS\n");
					break;
			...
			if (conn->key_type == HCI_LK_AUTH_COMBINATION_P256)
			{
				...
				printk(KERN_INFO "NIAP-CC: P-256 authentication key (FIPS-mode)\n");
			}
			if ((conn->type == ACL_LINK && ev->encrypt == 0x02) ||
			    conn->type == LE_LINK) {
				printk(KERN_INFO "NIAP-CC: AES-CCM encryption (FIPS-mode)\n");
	...
	if (!ev->status && ev->encrypt && conn->type == ACL_LINK) {
		...
		if (!(hdev->commands[20] & 0x10)) {
			/* NIAP-CC: BR/EDR change key size */
			//conn->enc_key_size = HCI_LINK_KEY_SIZE;
			conn->enc_key_size = smp_max_enc_key_size;
			printk(KERN_INFO "NIAP-CC: hci_encrypt_change_evt, key_size: %d\n", conn->enc_key_size);
			goto notify;
		}

		hci_req_init(&req, hdev);
		printk(KERN_INFO "NIAP-CC: sending read key size\n");
		...
		if (hci_req_run_skb(&req, read_enc_key_size_complete)) {
			...
			printk(KERN_INFO "NIAP-CC: sending read key size failed\n");
			//conn->enc_key_size = HCI_LINK_KEY_SIZE;
			conn->enc_key_size = smp_max_enc_key_size;
			goto notify;
		}
...
static void conn_set_key(struct hci_conn *conn, u8 key_type, u8 pin_len)
{
	...
	printk(KERN_INFO "NIAP-CC: conn_set_key\n");
	...
	case HCI_LK_COMBINATION:
		if (pin_len == 16)
			...
			conn->pending_sec_level = BT_SECURITY_MEDIUM;
		printk(KERN_INFO "NIAP-CC: Combination Key\n");
		break;
	case HCI_LK_UNAUTH_COMBINATION_P192:
			printk(KERN_INFO "NIAP-CC: Unauthenticated Combination Key generated from P-192\n");
			break;
	case HCI_LK_UNAUTH_COMBINATION_P256:
		conn->pending_sec_level = BT_SECURITY_MEDIUM;
		printk(KERN_INFO "NIAP-CC: Unauthenticated Combination Key generated from P-256\n");
		break;
	case HCI_LK_AUTH_COMBINATION_P192:
		printk(KERN_INFO "NIAP-CC: Authenticated Combination Key generated from P-192\n");
		conn->pending_sec_level = BT_SECURITY_HIGH;
		break;
	case HCI_LK_AUTH_COMBINATION_P256:
		printk(KERN_INFO "NIAP-CC: Authenticated Combination Key generated from P-256\n");
		conn->pending_sec_level = BT_SECURITY_FIPS;
		break;
	}
}
...
static void hci_link_key_request_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	BT_DBG("%s", hdev->name);
	printk(KERN_INFO "NIAP-CC: hci_link_key_request_evt\n");
